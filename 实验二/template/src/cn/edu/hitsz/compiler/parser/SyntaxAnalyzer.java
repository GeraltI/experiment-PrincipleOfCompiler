    ...
    public void run() {
        ...
        while(true){
            ...
            if(!tokensQueue.isEmpty()){
                //如果token队列非空,根据token队列头部和状态栈顶判断状态下一步动作
                Token token = tokensQueue.peek();
                Action action = table.getAction(status,token);
                //如果符号栈非空且符号栈栈顶为非终止符且符号栈栈顶goto状态不是Error,则执行goto
                if(!termsStack.empty() && termsStack.peek() instanceof NonTerminal && !table.getGoto(status,(NonTerminal)termsStack.peek()).isError()){
                    Status statusGoto = table.getGoto(status,(NonTerminal)termsStack.peek());
                    statusStack.push(statusGoto);
                }
                //如果不goto且动作为转移，则执行转移
                else if(action.getKind().equals(Action.ActionKind.Shift)) {
                    callWhenInShift(status,token);
                    statusStack.push(action.getStatus());
                    termsStack.push(tokensQueue.poll().getKind());
                }
                //如果不goto且动作为规约，则执行规约
                else if(action.getKind().equals(Action.ActionKind.Reduce)){
                ...
                //如果不goto且动作为接受，则执行接受
                else if(action.getKind().equals(Action.ActionKind.Accept)){
                    callWhenInAccept(status);
                    break;
                }
                //如果不goto且动作为错误，则执行错误
                else if(action.getKind().equals(Action.ActionKind.Error)){
                    throw new RuntimeException("Wrong!");
                }
            }
        }
